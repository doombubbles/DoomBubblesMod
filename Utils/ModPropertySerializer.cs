    ...
    public override T Read<T>(BinaryReader reader)
    {
        return Read(reader) switch
        {
            ...
            _ => throw new SerializationException(
                $"Result was of type {Read(reader).GetType().Name} when expected {typeof(T).Name}")
        };
    }
        ...
        switch (value)
        {
            ...
            default:
                throw new SerializationException(
                    $"Could not write value of type {value.GetType().Name} to packet when expected {typeof(TType).Name}");
        }
    }
...
{
    public override ModPlayer Read(BinaryReader reader)
    {
        var playerIndex = reader.ReadInt32();
        var modPlayerIndex = reader.ReadUInt16();
        
        return GetOrDefault(Main.player, playerIndex)?.GetModPlayer()
    }

    public override void Write(ModPacket packet, ModPlayer value)
    {
