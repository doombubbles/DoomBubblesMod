...
namespace DoomBubblesMod.Projectiles
{
    public class ManapireRestore : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (distance < 50f && projectile.position.X < Main.player[healTarget].position.X + (float)Main.player[healTarget].width && projectile.position.X + (float)projectile.width > Main.player[healTarget].position.X && projectile.position.Y < Main.player[healTarget].position.Y + (float)Main.player[healTarget].height && projectile.position.Y + (float)projectile.height > Main.player[healTarget].position.Y)
            {
                if (projectile.owner == Main.myPlayer)
                {
                    ...
                    } else if (!Main.player[Main.myPlayer].moonLeech && player.statLife < player.statLifeMax2 && !(player.lifeSteal <= 0f))
                    {
                        //life restore
            ...
            for (int num500 = 0; num500 < 3; num500 = num4 + 1)
            {
                ...
                int num503 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, mod.DustType("ManapireDust2"), 0f, 0f, 100, default(Color), 1.1f);
